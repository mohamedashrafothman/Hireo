(function($){
    ...
    function mainMap() {
      ...
      // Locations
		var locations = [];
		var map_pointers = JSON.parse($("#map").attr("data-map-pointers"));
		for (var i = 0; i < map_pointers.length; i++) {
			locations.push([`${locationData(map_pointers[i].jobUrl, map_pointers[i].companyLogo, map_pointers[i].companyName, map_pointers[i].jobTitle, map_pointers[i].verifiedBadge)}`, map_pointers[i].jobLat, map_pointers[i].jobLong, i, map_pointers[i].markerIcon]);
		}
