		...
		const [postsErr, posts] = await to(
			this.model
				.aggregate([
					{
						$match: {
							...(options.query?.q && { $or: [{ title: { $regex: options.query.q.split(" ").filter(Boolean).join("|") || "", $options: "i" } }, { content: { $regex: options.query.q.split(" ").filter(Boolean).join("|") || "", $options: "i" } }] }),
					...
					},
					{
						$project: {
							_id: 1,
							views_count: 1,
							zScore: { $cond: [{ $eq: ["$standard_deviation", 0] }, 0, { $divide: [{ $subtract: ["$views_count", "$average_views"] }, "$standard_deviation"] }] }
						}
					},
